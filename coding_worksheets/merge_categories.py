...
if __name__ == "__main__":
	...
	filename = 'IRR_data_resolved.csv'
	...
	df_ = df.loc[(df['qt-bad-ql-good'].isin([1,-1]))]
	# print df_
	print df_.shape

	df_ = df.loc[(df['zero-outcome'].isin([1,-1])) | (df['extreme-outcome'].isin([1,-1]))]
	print df_
	print len(df_.index.get_level_values('Student ID').unique())

	df_ = df.loc[(df['unified-extremes'].isin([1,-1])) | (df['unified-slope'].isin([1,-1])) | (df['unified-zero'].isin([1,-1]))]
	# print df_
	print len(df_.index.get_level_values('Student ID').unique())
	...
	# #to evaluate agreement on identified filter out all 2s and 3s from 'summary'
	# df1.loc[~df1['summary_1'].isin([1,-1]),'summary_1'] = 0
	# df2.loc[~df2['summary_2'].isin([1,-1]),'summary_2'] = 0
	# # print df2.loc[~df2['summary_2'].isin([1,-1]),'summary_2']
