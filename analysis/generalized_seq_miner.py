...
def count_use_per_group_per_bin(allfrequencies, frequencies_by_bin, num_bins, students_by_attribute, levels):
    '''
    '''
...
def get_sequence_use_by_timebin(action_seqs, time_coords, students_by_attribute, num_bins, attribute,
    ...
    counts = count_use_per_group_per_bin(cleaned_frequencies, frequencies_by_bin, num_bins, students_by_attribute, levels)
    return counts
...
def run_example():
    ...
    # For this example we have few students so we set the cut off to 0.
    cut_off = 0
    ...
    # Thus the last time bin will tend to contain shorter sequences and less sequences.
